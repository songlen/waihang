...
class index extends foreground {
	...
	public function init() {
		...
		if($_POST['dosubmit']){
			...
			// 更新对应语言的个别对应字段（如果存在）
			$update_language = $language == 'zh' ? 'en' : 'zh';
			$update_where = array(
				'member_id' => $userid,
			...
			if($resume_exist){
				$update_data = array(
					'sex' => $info['sex'],
					'political_outlook' => $info['political_outlook'],
					'marital_status' => $info['marital_status'],
					'diploma' => $info['diploma'],
					'swimming_ability' => $info['swimming_ability'],
					'living_province_id' => $info['living_province_id'],
			...
			$basicinfo = $this->member_resume_model->get_one(array('member_id'=>$userid, 'language'=>$language));

			// 获取省列表
			$linkage = pc_base::load_model('linkage_model');
			$where = "parentid=0 and linkageid != 1";
			$province = $linkage->select($where, 'linkageid, name, pinyin');
			// 获取现居住地城市

			$living_city_list = $linkage->select(array('parentid'=>$basicinfo['living_province_id']), 'linkageid, name, pinyin');
			$hukou_city_list = $linkage->select(array('parentid'=>$basicinfo['hukou_province_id']), 'linkageid, name, pinyin');
...
/*	public function changepinyin(){
		...
		foreach ($city as $item) {
			...
			// p($where);
			$linkage->update(array('pinyin'=>$item['pinyin']), $where);
		}
	}*/
	...
	public function ajaxGetCity(){
		...
		$linkage = pc_base::load_model('linkage_model');
		$city = $linkage->select(array('parentid'=>$province_id), 'linkageid, name, pinyin');

		if($city){
			die(json_encode(array('code'=>200, 'data'=>$city)));
		} else {
			error('error');
		}
	}
	...
	public function education_modify(){
		...
		if($_POST['dosubmit']){
			...
			if($language == 'zh'){
				...
				include template('member', 'education_modify_en');
			}
		}
	}
	...
	public function work_modify(){
		...
		if($_POST['dosubmit']){
			...
			if($language == 'zh'){
				...
				include template('member', 'work_modify_en');
			}
		}
	}
	...
	public function language_modify(){
		...
		if($_POST['dosubmit']){
			...
			if($language == 'zh'){
				...
				include template('member', 'language_modify_en');
			}
		}
	}
	...
	public function pic(){
		...
		if($_POST['dosubmit']){
			...
			if($type == 'head'){
				// 2寸图片尺寸 100*150
				if($imgsize[0] > 100) error('图片宽度不能超过100px');
			...
			$a = $attachment->upload('file', 'jpg|png|jpeg|gif', 500*1024);
	 ...
	 */
	public function public_sendPhoneCode(){
		$phone = $_GET['phone'];
		$type = $_GET['type'];
		...
		// 如果找回密码，检测是否注册
		if($type == 'forget'){
			if(!$is_register) error('该手机号未注册');
		}

		$sms = pc_base::load_sys_class('sms');
		$sms->send($phone, $type);
	}
	...
	public function previewResume(){
		...
		$language =isset($_GET['l']) ? $_GET['l'] : 'zh';
		...
		$SEO = seo(SITEID);
		if($language == 'zh'){
			include template('member', 'previewResume');
		} else if($language == 'en') {
			include template('member', 'previewResume_en');
		}
	}
	...
	public function generateBarCode($code=''){
		include('statics/plugin/php-barcode-generator-master/BarcodeGenerator.php');
		include('statics/plugin/php-barcode-generator-master/BarcodeGeneratorJPG.php');
	 	$generatorJPG = new Picqer\Barcode\BarcodeGeneratorJPG();
	 
	 	// header('Content-type:image/jpeg');
	 	// echo $generatorJPG->getBarcode('081231723897', $generatorJPG::TYPE_CODE_128, 2, 100);
	 	return '<img src="data:image/jpeg;base64,'
	 	.base64_encode($generatorJPG->getBarcode($code, $generatorJPG::TYPE_CODE_128, 2, 30))
	 	.'">';
	}
	...
	public function lesson(){
		...
		$order_model = pc_base::load_model('train_order_model');
		$sql = "select o.id, o.lesson_id, o.inputtime, o.status, l.title, l.price  from phpcms_train_order as o left join phpcms_train_lesson as l on o.lesson_id=l.id where o.user_id={$userid}";
