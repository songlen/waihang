...
<table width="100%" cellspacing="0" class="search-form">
    <tbody>
		<tr>
		<td><div class="explain-col"><?php echo L('module')?>: <?php echo form::select($module_arr,'','name="search[module]"',$default)?> 用户名:  <input type="text" value=<?php echo input('search')['username'];?> class="input-text" name="search[username]" size='10'>  时 间:  <?php echo form::date('search[start_time]',input('search')['start_time'],'1')?> 至   <?php echo form::date('search[end_time]',input('search')['end_time'],'1')?>    <input type="submit" value="确定搜索" class="button" name="dosubmit">
		</div>
		</td>
		</tr>
    </tbody>
